...
- объяснить `transient vs permanent failure` своими словами и связать с backend-сценарием;
- объяснить `exponential backoff` своими словами и связать с backend-сценарием;
- объяснить `jitter` своими словами и связать с backend-сценарием;
...
В теме **Retry and backoff** важно уверенно объяснять следующие части:

### transient vs permanent failure

Для `transient vs permanent failure` проследи delivery от commit до side effect, включая duplicate, retry и idempotency.

### exponential backoff

Для `exponential backoff` проследи delivery от commit до side effect, включая duplicate, retry и idempotency.

### jitter

Для `jitter` проследи delivery от commit до side effect, включая duplicate, retry и idempotency.
...
1. Объясни **Retry and backoff** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Retry and backoff**;
