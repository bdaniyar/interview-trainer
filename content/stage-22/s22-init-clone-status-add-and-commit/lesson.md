...
- объяснить `init, clone, status, add and commit` своими словами и связать с backend-сценарием;
...
В теме **init, clone, status, add and commit** важно уверенно объяснять следующие части:

### init, clone, status, add and commit

Для `init, clone, status, add and commit` сначала назови изменяемое состояние Git: working tree, index, branch pointer или shared history.
...
1. Объясни **init, clone, status, add and commit** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **init, clone, status, add and commit**;
