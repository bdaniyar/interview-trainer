...
- объяснить `ignoring future files does not remove tracked/history content` своими словами и связать с backend-сценарием;
- объяснить `leaked secret must be rotated.` своими словами и связать с backend-сценарием;
...
В теме **`.gitignore` and secrets** важно уверенно объяснять следующие части:

### ignoring future files does not remove tracked/history content

Для `ignoring future files does not remove tracked/history content` сначала назови изменяемое состояние Git: working tree, index, branch pointer или shared history.

### leaked secret must be rotated

Для `leaked secret must be rotated` сначала назови изменяемое состояние Git: working tree, index, branch pointer или shared history.
...
1. Объясни **`.gitignore` and secrets** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **`.gitignore` and secrets**;
