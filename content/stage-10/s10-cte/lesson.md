...
- объяснить `multiple stages` своими словами и связать с backend-сценарием;
...
В теме **CTE** важно уверенно объяснять следующие части:

### `WITH`

Для ``WITH`` сначала определи grain/cardinality результата, затем NULL и ordering semantics.

### readability

Для `readability` сначала определи grain/cardinality результата, затем NULL и ordering semantics.

### multiple stages

Для `multiple stages` сначала определи grain/cardinality результата, затем NULL и ordering semantics.

### planner/version caveats

Для `planner/version caveats` сначала определи grain/cardinality результата, затем NULL и ordering semantics.
...
1. Объясни **CTE** по схеме «определение → механизм → пример → ограничение».
...
Expected columns: id, depth. Comparison: ordered.
