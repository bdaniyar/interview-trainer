...
- объяснить ``IS NULL`` своими словами и связать с backend-сценарием;
- объяснить ``IS NOT NULL`` своими словами и связать с backend-сценарием;
- объяснить ``NULL` comparisons` своими словами и связать с backend-сценарием;
...
В теме **NULL and three-valued logic** важно уверенно объяснять следующие части:
...
Для ``COALESCE`` сначала определи grain/cardinality результата, затем NULL и ordering semantics.
...
1. Объясни **NULL and three-valued logic** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **NULL and three-valued logic**;
