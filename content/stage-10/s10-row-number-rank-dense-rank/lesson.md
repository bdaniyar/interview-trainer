...
- объяснить `ties` своими словами и связать с backend-сценарием;
- объяснить `top-N per group.` своими словами и связать с backend-сценарием;
...
В теме **`ROW_NUMBER`, `RANK`, `DENSE_RANK`** важно уверенно объяснять следующие части:

### ties

Для `ties` сначала определи grain/cardinality результата, затем NULL и ordering semantics.
...
1. Объясни **`ROW_NUMBER`, `RANK`, `DENSE_RANK`** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **`ROW_NUMBER`, `RANK`, `DENSE_RANK`**;
