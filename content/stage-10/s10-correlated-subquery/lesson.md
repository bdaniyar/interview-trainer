...
- объяснить `per-row evaluation mental model` своими словами и связать с backend-сценарием;
- объяснить `optimizer caveat` своими словами и связать с backend-сценарием;
- объяснить `alternatives.` своими словами и связать с backend-сценарием;
...
В теме **Correlated subquery** важно уверенно объяснять следующие части:

### per-row evaluation mental model

Для `per-row evaluation mental model` сначала определи grain/cardinality результата, затем NULL и ordering semantics.

### optimizer caveat

Для `optimizer caveat` сначала определи grain/cardinality результата, затем NULL и ordering semantics.

### alternatives

Для `alternatives` сначала определи grain/cardinality результата, затем NULL и ordering semantics.
...
1. Объясни **Correlated subquery** по схеме «определение → механизм → пример → ограничение».
...
Expected columns: id, user_id. Comparison: ordered.
...
- дай точное определение **Correlated subquery**;
