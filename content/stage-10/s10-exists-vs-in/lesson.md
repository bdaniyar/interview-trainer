...
- объяснить `existence semantics` своими словами и связать с backend-сценарием;
- объяснить `NULL caveats` своими словами и связать с backend-сценарием;
- объяснить `no simplistic universal performance rule.` своими словами и связать с backend-сценарием;
...
В теме **EXISTS vs IN** важно уверенно объяснять следующие части:

### existence semantics

Для `existence semantics` сначала определи grain/cardinality результата, затем NULL и ordering semantics.
...
1. Объясни **EXISTS vs IN** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **EXISTS vs IN**;
