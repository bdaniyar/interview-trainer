...
- объяснить `durability` своими словами и связать с backend-сценарием;
- объяснить `retries` своими словами и связать с backend-сценарием;
...
В теме **Background jobs** важно уверенно объяснять следующие части:

### latency

Для `latency` начни с requirements/source of truth и только затем добавляй component под измеримый failure mode.

### durability

Для `durability` начни с requirements/source of truth и только затем добавляй component под измеримый failure mode.

### retries

Для `retries` начни с requirements/source of truth и только затем добавляй component под измеримый failure mode.
...
1. Объясни **Background jobs** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Background jobs**;
