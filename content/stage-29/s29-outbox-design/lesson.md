...
- объяснить `atomic write` своими словами и связать с backend-сценарием;
- объяснить `worker claim` своими словами и связать с backend-сценарием;
- объяснить `retry` своими словами и связать с backend-сценарием;
...
В теме **Outbox design** важно уверенно объяснять следующие части:

### atomic write

Для `atomic write` начни с requirements/source of truth и только затем добавляй component под измеримый failure mode.

### worker claim

Для `worker claim` начни с requirements/source of truth и только затем добавляй component под измеримый failure mode.
...
1. Объясни **Outbox design** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Outbox design**;
