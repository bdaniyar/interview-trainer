...
- объяснить `stored/reflected` своими словами и связать с backend-сценарием;
- объяснить `output encoding` своими словами и связать с backend-сценарием;
- объяснить `relevance to API-returned user content.` своими словами и связать с backend-сценарием;
...
В теме **XSS** важно уверенно объяснять следующие части:

### stored/reflected

Для `stored/reflected` назови threat, trust boundary, server-side check и безопасный failure response.

### output encoding

Текстовый `str` хранит Unicode, а `bytes` — конкретные байты; encode/decode всегда выполняют на явной границе с выбранной encoding и error policy.

### relevance to API-returned user content

Для `relevance to API-returned user content` назови threat, trust boundary, server-side check и безопасный failure response.
...
1. Объясни **XSS** по схеме «определение → механизм → пример → ограничение».
