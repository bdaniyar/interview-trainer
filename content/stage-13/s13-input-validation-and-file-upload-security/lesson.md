...
- объяснить `size` своими словами и связать с backend-сценарием;
- объяснить `type` своими словами и связать с backend-сценарием;
- объяснить `filename/key` своими словами и связать с backend-сценарием;
...
В теме **Input validation and file upload security** важно уверенно объяснять следующие части:

### size

Для `size` назови threat, trust boundary, server-side check и безопасный failure response.

### type

Для `type` назови threat, trust boundary, server-side check и безопасный failure response.

### filename/key

Для `filename/key` назови threat, trust boundary, server-side check и безопасный failure response.

### content inspection limitations

Для `content inspection limitations` назови threat, trust boundary, server-side check и безопасный failure response.
...
Для `orphan cleanup` назови threat, trust boundary, server-side check и безопасный failure response.
...
1. Объясни **Input validation and file upload security** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Input validation and file upload security**;
