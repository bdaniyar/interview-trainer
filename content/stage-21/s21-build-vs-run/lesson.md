...
- объяснить `Build vs run` своими словами и связать с backend-сценарием;
...
В теме **Build vs run** важно уверенно объяснять следующие части:

### Build vs run

Для `Build vs run` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.
...
1. Объясни **Build vs run** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Build vs run**;
