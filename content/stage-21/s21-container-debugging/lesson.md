...
- объяснить `logs` своими словами и связать с backend-сценарием;
- объяснить `inspect env` своими словами и связать с backend-сценарием;
- объяснить `process` своими словами и связать с backend-сценарием;
...
В теме **Container debugging** важно уверенно объяснять следующие части:
...
Для `inspect env` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.
...
Для `port` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.

### DNS

Для `DNS` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.
...
Для `migrations` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.
...
1. Объясни **Container debugging** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Container debugging**;
