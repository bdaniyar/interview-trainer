...
- объяснить ``.dockerignore`` своими словами и связать с backend-сценарием;
...
В теме **`.dockerignore`** важно уверенно объяснять следующие части:
...
1. Объясни **`.dockerignore`** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **`.dockerignore`**;
