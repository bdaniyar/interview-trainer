...
- объяснить `named volume` своими словами и связать с backend-сценарием;
- объяснить `bind mount` своими словами и связать с backend-сценарием;
- объяснить `persistence` своими словами и связать с backend-сценарием;
...
В теме **Volumes** важно уверенно объяснять следующие части:

### named volume

Для `named volume` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.

### bind mount

Для `bind mount` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.

### persistence

Для `persistence` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.

### source mount pitfalls

Для `source mount pitfalls` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.
...
1. Объясни **Volumes** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Volumes**;
