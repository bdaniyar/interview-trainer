...
- объяснить `dependency files first` своими словами и связать с backend-сценарием;
- объяснить `source later` своими словами и связать с backend-сценарием;
- объяснить `cache invalidation.` своими словами и связать с backend-сценарием;
...
В теме **Layers and cache** важно уверенно объяснять следующие части:
...
Для `source later` раздели image/build-time и container/runtime, затем проверь DNS, ports, mounts и lifecycle.
...
1. Объясни **Layers and cache** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Layers and cache**;
