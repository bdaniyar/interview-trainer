...
- объяснить `isolated test DB/schema` своими словами и связать с backend-сценарием;
- объяснить `transaction rollback or recreate` своими словами и связать с backend-сценарием;
...
В теме **Database tests** важно уверенно объяснять следующие части:

### isolated test DB/schema

Для `isolated test DB/schema` сформулируй observable contract, isolation boundary и failure, который обязан поймать test.
...
Для `migrations` сформулируй observable contract, isolation boundary и failure, который обязан поймать test.

### no production DB

Для `no production DB` сформулируй observable contract, isolation boundary и failure, который обязан поймать test.
...
1. Объясни **Database tests** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Database tests**;
