...
**short access token.** `short access token` защищается по реализованному flow: проблема, принятое решение, компромисс, режим отказа и test/metric.
...
**rotation.** `rotation` защищается по реализованному flow: проблема, принятое решение, компромисс, режим отказа и test/metric.

**revocation.** `revocation` защищается по реализованному flow: проблема, принятое решение, компромисс, режим отказа и test/metric.

**reuse detection.** `reuse detection` защищается по реализованному flow: проблема, принятое решение, компромисс, режим отказа и test/metric.
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `short access token` до запуска.

**B · Найди ошибку.** Найди нарушение `server-side refresh session` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про JWT refresh sessions за 60 секунд: определение, механизм, пример, ограничение.
...
**Критерии ответа:** Server session, rotation/revocation, short access.
...
Сделай короткую письменную практику по теме **JWT refresh sessions**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
