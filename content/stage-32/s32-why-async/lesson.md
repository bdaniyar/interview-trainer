...
- объяснить `PostgreSQL/Redis/object storage/WebSocket are I/O-bound` своими словами и связать с backend-сценарием;
- объяснить `event loop serves other work while waiting` своими словами и связать с backend-сценарием;
- объяснить `CPU-heavy work must leave event loop.` своими словами и связать с backend-сценарием;
...
В теме **Why async?** важно уверенно объяснять следующие части:
...
1. Объясни **Why async?** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Why async?**;
