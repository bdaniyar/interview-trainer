...
**несколько головных ревизий.** `multiple heads` является частью версионированного перехода схемы; безопасный вариант учитывает upgrade, совместимость при развёртывании, заполнение существующих данных и rollback.

**объединяющая ревизия.** `merge revision` является частью версионированного перехода схемы; безопасный вариант учитывает upgrade, совместимость при развёртывании, заполнение существующих данных и rollback.

**командный процесс работы.** `team workflow` является частью версионированного перехода схемы; безопасный вариант учитывает upgrade, совместимость при развёртывании, заполнение существующих данных и rollback.
...
- несколько головных ревизий
- объединяющая ревизия
- командный процесс работы
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `multiple heads` до запуска.

**B · Найди ошибку.** Найди нарушение `merge revision` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про Migration history conflicts за 60 секунд: определение, механизм, пример, ограничение.
...
Сделай короткую письменную практику по теме **Migration history conflicts**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
