...
- объяснить `multiple heads` своими словами и связать с backend-сценарием;
- объяснить `merge revision` своими словами и связать с backend-сценарием;
- объяснить `team workflow.` своими словами и связать с backend-сценарием;
...
В теме **Migration history conflicts** важно уверенно объяснять следующие части:

### multiple heads

Для `multiple heads` опиши проверяемый schema transition и отдельно риски upgrade, deploy compatibility и rollback.

### merge revision

Для `merge revision` опиши проверяемый schema transition и отдельно риски upgrade, deploy compatibility и rollback.

### team workflow

Для `team workflow` опиши проверяемый schema transition и отдельно риски upgrade, deploy compatibility и rollback.
...
1. Объясни **Migration history conflicts** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Migration history conflicts**;
