...
- объяснить `filter` своими словами и связать с backend-сценарием;
...
В теме **Django ORM query basics** важно уверенно объяснять следующие части:

### filter

Для `filter` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.

### get

Для `get` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.

### create

Для `create` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.

### update

Для `update` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.

### delete

Для `delete` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.

### lazy QuerySet

Для `lazy QuerySet` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.
...
1. Объясни **Django ORM query basics** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Django ORM query basics**;
