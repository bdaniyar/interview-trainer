...
- объяснить `create/update` своими словами и связать с backend-сценарием;
- объяснить `model serializer` своими словами и связать с backend-сценарием;
...
В теме **DRF serializers** важно уверенно объяснять следующие части:

### validation

Для `validation` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.

### create/update

Для `create/update` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.

### model serializer

Для `model serializer` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.

### nested data caution

Для `nested data caution` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.
...
1. Объясни **DRF serializers** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **DRF serializers**;
