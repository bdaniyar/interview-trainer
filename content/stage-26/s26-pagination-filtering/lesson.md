...
- объяснить `Pagination/filtering` своими словами и связать с backend-сценарием;
...
В теме **Pagination/filtering** важно уверенно объяснять следующие части:

### Pagination/filtering

Для `Pagination/filtering` сопоставь Django/DRF abstraction с request, ORM query count, validation и permissions.
...
1. Объясни **Pagination/filtering** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Pagination/filtering**;
