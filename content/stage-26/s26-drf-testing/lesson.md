...
**DRF testing.** `DRF testing` входит в Django/DRF обработка запроса и влияет на ORM query count, validation, permissions или сериализация ответа.
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `DRF testing` до запуска.

**B · Найди ошибку.** Найди нарушение `DRF testing` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про DRF testing за 60 секунд: определение, механизм, пример, ограничение.
...
Сделай короткую письменную практику по теме **DRF testing**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
