...
**время и память.** `time vs space` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.

**доминирующий член сложности.** `dominant term` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.

**common collection operations.** `common collection operations` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `time vs space` до запуска.

**B · Найди ошибку.** Найди нарушение `dominant term` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про Big O за 60 секунд: определение, механизм, пример, ограничение.
...
Сделай короткую письменную практику по теме **Big O**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
