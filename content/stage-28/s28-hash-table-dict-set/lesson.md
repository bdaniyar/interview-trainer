...
**средняя сложность.** `average complexity` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.

**понимание коллизий.** `collisions awareness` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.

**membership/deduplication.** `membership/deduplication` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `average complexity` до запуска.

**B · Найди ошибку.** Найди нарушение `collisions awareness` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про Hash table/dict/set за 60 секунд: определение, механизм, пример, ограничение.
...
Сделай короткую письменную практику по теме **Hash table/dict/set**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
