...
**brackets.** `brackets` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.

**undo.** `undo` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.

**Основы обхода в глубину.** `DFS basics` определяет операцию со структурой данных с конкретной временной и пространственной сложностью и набором граничных случаев.
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `brackets` до запуска.

**B · Найди ошибку.** Найди нарушение `undo` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про Stack за 60 секунд: определение, механизм, пример, ограничение.
...
Сделай короткую письменную практику по теме **Stack**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
