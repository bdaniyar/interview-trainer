...
- объяснить `Status codes, headers and cookies` своими словами и связать с backend-сценарием;
...
В теме **Status codes, headers and cookies** важно уверенно объяснять следующие части:

### Status codes, headers and cookies

Status code сообщает результат HTTP operation: 2xx success, 4xx client/request state, 5xx server failure; error body добавляет стабильный machine-readable code.
...
1. Объясни **Status codes, headers and cookies** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Status codes, headers and cookies**;
