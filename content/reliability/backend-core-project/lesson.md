...
**Basic request path.** `Basic request path` является компонентом system design только при наличии требования, source of truth и измеримого режим отказа.
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `Basic request path` до запуска.

**B · Найди ошибку.** Найди нарушение `Basic request path` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про Basic request path за 60 секунд: определение, механизм, пример, ограничение.
...
**Критерии ответа:** Boundaries, timeout, request id, source of truth.
...
Сделай короткую письменную практику по теме **Basic request path**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
