...
- объяснить `Basic request path` своими словами и связать с backend-сценарием;
...
В теме **Basic request path** важно уверенно объяснять следующие части:

### Basic request path

Для `Basic request path` начни с requirements/source of truth и только затем добавляй component под измеримый failure mode.
...
1. Объясни **Basic request path** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Basic request path**;
