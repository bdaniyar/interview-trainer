...
- объяснить `hierarchy` своими словами и связать с backend-сценарием;
- объяснить `useful context` своими словами и связать с backend-сценарием;
- объяснить `mapping to HTTP errors` своими словами и связать с backend-сценарием;
...
В теме **Custom exceptions** важно уверенно объяснять следующие части:

### hierarchy

Для `hierarchy` опиши protocol: кто инициирует шаг, какое состояние сохраняется, как выглядит завершение и error path.

### useful context

Для `useful context` опиши protocol: кто инициирует шаг, какое состояние сохраняется, как выглядит завершение и error path.

### mapping to HTTP errors

Для `mapping to HTTP errors` опиши protocol: кто инициирует шаг, какое состояние сохраняется, как выглядит завершение и error path.

### avoiding exception-driven normal flow

Для `avoiding exception-driven normal flow` опиши protocol: кто инициирует шаг, какое состояние сохраняется, как выглядит завершение и error path.
...
1. Объясни **Custom exceptions** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Custom exceptions**;
