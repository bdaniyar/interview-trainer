...
- объяснить `event loop plugin/config` своими словами и связать с backend-сценарием;
- объяснить `AsyncClient` своими словами и связать с backend-сценарием;
- объяснить `deterministic timeouts` своими словами и связать с backend-сценарием;
...
В теме **Async tests** важно уверенно объяснять следующие части:
...
Для `AsyncClient` сформулируй observable contract, isolation boundary и failure, который обязан поймать test.

### deterministic timeouts

Для `deterministic timeouts` сформулируй observable contract, isolation boundary и failure, который обязан поймать test.

### no sleep-heavy tests

Для `no sleep-heavy tests` сформулируй observable contract, isolation boundary и failure, который обязан поймать test.
...
1. Объясни **Async tests** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Async tests**;
