...
- объяснить ``raise ... from ...`` своими словами и связать с backend-сценарием;
- объяснить `preserving root cause` своими словами и связать с backend-сценарием;
- объяснить `translating infrastructure errors into domain/API errors.` своими словами и связать с backend-сценарием;
...
В теме **Exception chaining** важно уверенно объяснять следующие части:

### `raise ... from ...`

Для ``raise ... from ...`` опиши protocol: кто инициирует шаг, какое состояние сохраняется, как выглядит завершение и error path.

### preserving root cause

Для `preserving root cause` опиши protocol: кто инициирует шаг, какое состояние сохраняется, как выглядит завершение и error path.

### translating infrastructure errors into domain/API errors

Для `translating infrastructure errors into domain/API errors` опиши protocol: кто инициирует шаг, какое состояние сохраняется, как выглядит завершение и error path.
...
1. Объясни **Exception chaining** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Exception chaining**;
