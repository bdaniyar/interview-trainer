...
**length.** `length` влияет на Pydantic v2 validation/serialization и должен различать missing, явное значение null, некорректные входные данные и представление результата.

**range.** `range` влияет на Pydantic v2 validation/serialization и должен различать missing, явное значение null, некорректные входные данные и представление результата.

**pattern.** `pattern` влияет на Pydantic v2 validation/serialization и должен различать missing, явное значение null, некорректные входные данные и представление результата.
...
**Field.** `Field` влияет на Pydantic v2 validation/serialization и должен различать missing, явное значение null, некорректные входные данные и представление результата.
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `length` до запуска.

**B · Найди ошибку.** Найди нарушение `range` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про Field constraints за 60 секунд: определение, механизм, пример, ограничение.
...
> Field constraints — тема, в которой я сначала фиксирую `length`, затем объясняю `range` на коротком примере. Ключевой механизм: Проверь четыре состояния: missing, явное значение null, invalid type/value и сериализованный результат. Главная практическая ошибка — Смешать missing и явное значение null либо считать coercion бизнес-валидацией.
