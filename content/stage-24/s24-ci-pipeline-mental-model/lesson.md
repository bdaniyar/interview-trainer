...
- объяснить `checkout` своими словами и связать с backend-сценарием;
- объяснить `install` своими словами и связать с backend-сценарием;
- объяснить `lint` своими словами и связать с backend-сценарием;
...
В теме **CI pipeline mental model** важно уверенно объяснять следующие части:

### checkout

Для `checkout` определи reproducible quality gate, trigger, artifact и безопасное управление secret.

### install

Для `install` определи reproducible quality gate, trigger, artifact и безопасное управление secret.

### lint

Для `lint` определи reproducible quality gate, trigger, artifact и безопасное управление secret.

### test

Для `test` определи reproducible quality gate, trigger, artifact и безопасное управление secret.
...
1. Объясни **CI pipeline mental model** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **CI pipeline mental model**;
