...
- объяснить `client → Nginx/proxy → application` своими словами и связать с backend-сценарием;
- объяснить `TLS termination` своими словами и связать с backend-сценарием;
- объяснить `headers` своими словами и связать с backend-сценарием;
...
В теме **Reverse proxy** важно уверенно объяснять следующие части:

### client → Nginx/proxy → application

Для `client → Nginx/proxy → application` зафиксируй observable HTTP contract: request semantics, response status/body и повтор запроса.
...
Для `load distribution` зафиксируй observable HTTP contract: request semantics, response status/body и повтор запроса.
...
1. Объясни **Reverse proxy** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Reverse proxy**;
