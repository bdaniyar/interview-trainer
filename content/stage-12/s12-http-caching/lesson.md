...
- объяснить `Cache-Control` своими словами и связать с backend-сценарием;
- объяснить `ETag` своими словами и связать с backend-сценарием;
- объяснить `conditional request` своими словами и связать с backend-сценарием;
...
В теме **HTTP caching** важно уверенно объяснять следующие части:
...
Для `ETag` зафиксируй observable HTTP contract: request semantics, response status/body и повтор запроса.

### conditional request

Для `conditional request` зафиксируй observable HTTP contract: request semantics, response status/body и повтор запроса.
...
1. Объясни **HTTP caching** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **HTTP caching**;
