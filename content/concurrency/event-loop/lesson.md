...
- объяснить `readiness` своими словами и связать с backend-сценарием;
- объяснить `callbacks/tasks` своими словами и связать с backend-сценарием;
...
В теме **Event loop** важно уверенно объяснять следующие части:
...
Для `readiness` проследи coroutine/task по await points, cancellation и cleanup, не предполагая отдельный thread.

### callbacks/tasks

Для `callbacks/tasks` проследи coroutine/task по await points, cancellation и cleanup, не предполагая отдельный thread.
...
1. Объясни **Event loop** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Event loop**;
