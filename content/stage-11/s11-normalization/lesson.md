...
- объяснить `duplication` своими словами и связать с backend-сценарием;
- объяснить `update anomalies` своими словами и связать с backend-сценарием;
- объяснить `1NF/2NF/3NF at practical Junior depth` своими словами и связать с backend-сценарием;
...
В теме **Normalization** важно уверенно объяснять следующие части:

### duplication

Для `duplication` назови защищаемый invariant, concurrent transaction и evidence из constraint или query plan.

### update anomalies

Для `update anomalies` назови защищаемый invariant, concurrent transaction и evidence из constraint или query plan.

### 1NF/2NF/3NF at practical Junior depth

Для `1NF/2NF/3NF at practical Junior depth` назови защищаемый invariant, concurrent transaction и evidence из constraint или query plan.

### deliberate denormalization only with reason

Для `deliberate denormalization only with reason` назови защищаемый invariant, concurrent transaction и evidence из constraint или query plan.
...
1. Объясни **Normalization** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Normalization**;
