...
**self-introduction.** `self-introduction` раскрывается через context, личное действие, измеримый результат и конкретный дополнительный вопрос без выдуманных деталей.

**StudyHub summary.** `StudyHub summary` раскрывается через context, личное действие, измеримый результат и конкретный дополнительный вопрос без выдуманных деталей.
...
**explaining async/transaction компромисс.** Transaction задаёт атомарную границу: либо все связанные изменения становятся видимыми, либо выполняется rollback.

**PR description.** `PR description` раскрывается через context, личное действие, измеримый результат и конкретный дополнительный вопрос без выдуманных деталей.
...
- StudyHub summary
- explaining a bug
- explaining async/transaction компромисс
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `self-introduction` до запуска.

**B · Найди ошибку.** Найди нарушение `StudyHub summary` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про English project explanation за 60 секунд: определение, механизм, пример, ограничение.
...
Сделай короткую письменную практику по теме **English project explanation**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
