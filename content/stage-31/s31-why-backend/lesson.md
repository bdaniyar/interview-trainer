...
- объяснить `Why backend?` своими словами и связать с backend-сценарием;
...
В теме **Why backend?** важно уверенно объяснять следующие части:

### Why backend?

Для `Why backend?` подготовь ответ на 60–90 секунд: context, личное действие, результат и проверяемый follow-up.
...
1. Объясни **Why backend?** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Why backend?**;
