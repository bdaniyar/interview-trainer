...
- объяснить `cache unavailable → DB fallback` своими словами и связать с backend-сценарием;
- объяснить `rate limiter/session behavior depends on risk` своими словами и связать с backend-сценарием;
- объяснить `thundering herd basics` своими словами и связать с backend-сценарием;
...
В теме **Redis failure scenarios** важно уверенно объяснять следующие части:
...
Для `thundering herd basics` определи Redis key/value, TTL, invalidation, concurrency и fallback при outage.

### stale values

Для `stale values` определи Redis key/value, TTL, invalidation, concurrency и fallback при outage.
...
1. Объясни **Redis failure scenarios** по схеме «определение → механизм → пример → ограничение».
...
- дай точное определение **Redis failure scenarios**;
