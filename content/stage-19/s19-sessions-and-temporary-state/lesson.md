...
**состояние обновления токена, сброса пароля и ограничения частоты запросов.** Пароль хранят через специализированный медленный password hash с солью, а не через быстрый общий hash.
...
- состояние обновления токена, сброса пароля и ограничения частоты запросов
- TTL
...
**A · Предсказание результата/reasoning.** Предскажи результат минимального примера для `refresh/password reset/rate-limit state` до запуска.

**B · Найди ошибку.** Найди нарушение `TTL` и объясни конкретное последствие.

**E · Ответ на собеседовании.** Дай ответ про Sessions and temporary state за 60 секунд: определение, механизм, пример, ограничение.
...
Сделай короткую письменную практику по теме **Sessions and temporary state**: реши один пункт из раздела «Практика», затем сравни своё объяснение с хорошим ответом уровня Junior. Для этого урока автоматические скрытые тесты не требуются.
